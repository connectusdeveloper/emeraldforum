...
class Select extends NativeSelect
{
    public function __construct(
        ...
        if ('string' === gettype($template)) {
            $this->template = ['name' => $template];
        }

        if ('string' === gettype($asyncData) || null === $asyncData) {
            $this->asyncData = [
                'api'         => $asyncData,
                'method'      => 'GET',
                'params'      => [],
    ...
    public function optionsToArray(): array
    {
        return $this->options
            ->map(function ($rawOption, $index): array {
                ...
                if ($this->optionValue && 'value' !== $this->optionValue) {
                    unset($option[$this->optionValue]);
                }

                if ($this->optionLabel && 'label' !== $this->optionLabel) {
                    unset($option[$this->optionLabel]);
                }

                if ($this->optionDescription && 'description' !== $this->optionDescription) {
